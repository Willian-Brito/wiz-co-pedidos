...
{
    Task<Result<PagedResult<PedidoDTO>>> GetAllPaged(int page, int pageSize);
    Task<Result<PedidoDTO?>> GetByIdAsync(Guid id);
    Task<Result<PagedResult<PedidoDTO>>> GetByStatusPaged(StatusPedido? status, int page, int pageSize);
}